...
The method report is `REPORT.md`. It is published as markdown; `tools/make_pdf.sh`
renders it to PDF on demand and checks that the two agree. No rendered copy is kept
here while the organizers' answer on preprints is outstanding.

Released under the MIT license, as required by challenge rule 7. The accompanying
method report is the one submitted under rule 6, extended after the submission; the
additions are listed at the top of `REPORT.md`.

## What is worth taking from here

Our score on this task is close to the random baseline, and the useful part of this
repository is probably not the method. It is the record of what could not be established.

Eleven sets of pretrained weights, seven read-outs, the choice of layer and every component
combination we tried are in `results/`, cell by cell, each classified by a rule written down
before the numbers were computed. One change survived. At 158 positive training images most
of the rest are differences smaller than the intervals around them, so the honest report is
that we could not separate them. That is a fact about the sample size as much as about the
methods, and it is the kind of result that usually goes unpublished.

If you have more data, or you fine-tune rather than freeze, this is meant as a map of what
we could not resolve rather than a list of what does not work.

<!-- BEGIN verdict-table (generated by tools/make_tables.py; do not edit by hand) -->
| What was varied | families | comparisons | better | worse | conflict | could not separate |
|---|---:|---:|---:|---:|---:|---:|
| The choice of pretrained weights, and what it interacts with | 3 | 674 | 38 | 54 | 13 | 569 (84%) |
| How features come off the network | 3 | 677 | 53 | 42 | 33 | 549 (81%) |
| How the head is built | 5 | 218 | &mdash; | 123 | 4 | 91 (42%) |
| How 49 positions become one score | 1 | 33 | 3 | &mdash; | 1 | 29 (88%) |
| Preprocessing and invariance | 3 | 222 | &mdash; | 43 | 4 | 175 (79%) |
| How the configuration is assembled | 1 | 11 | 1 | 3 | 5 | 2 (18%) |
| **All of it** | **16** | **1835** | **95** | **265** | **60** | **1415 (77%)** |
<!-- END verdict-table -->

<!-- BEGIN could-not-separate (generated by tools/make_tables.py; do not edit by hand) -->
77% of the comparisons (1415 of 1835) landed in neither direction: the paired interval failed to exclude zero in both cross-center directions, so the comparison settled nothing.
<!-- END could-not-separate -->

A row here can hold several hundred comparisons for three families, because the eleven
sets of pretrained weights are an axis *inside* most families rather than a family of
their own; each family crosses those eleven with something else.
[`docs/08_what_was_tested.md`](docs/08_what_was_tested.md) lists every axis and every
level that was actually compared.

The four columns fold the seven verdict classes used elsewhere in this repository:
`better` is A+ and B+ together, `worse` is A- and B-, `conflict` is E, where the two
cross-center directions disagree in sign, and `could not separate` is C and D.
The seven-class breakdown and the rule behind each verdict are in
[`docs/07_verdict_counts.md`](docs/07_verdict_counts.md). One row name is easy to misread:
"A-class components" are the components that had themselves been classified A in an
earlier grid; the letters A+ through E are a separate scheme. Every count is recomputed from `results/` by `src/verdict.py` and
checked against the published tables before it is written; `tools/make_tables.py`
regenerates this table and the long one.
...
| Path | Contents |
|---|---|
| `REPORT.md` | The method report submitted under rule 6, extended after the submission. Its opening paragraph lists what was added and when. |
...
```bash
pip install -r container/requirements.txt -r requirements-analysis.txt
