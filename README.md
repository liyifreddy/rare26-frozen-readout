...
Four in five comparisons landed in neither direction: the paired interval failed to
exclude zero in both cross-center directions, so the comparison settled nothing.
...
`better` is A+ and B+ together, `worse` is A- and B-, and `could not separate` is C and D.
...
earlier grid, not a verdict class. Every count is recomputed from `results/` by `src/verdict.py` and
